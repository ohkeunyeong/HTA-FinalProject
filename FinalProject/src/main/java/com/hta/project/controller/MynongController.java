...
public class MynongController {
	private static final Logger logger = LoggerFactory.getLogger(MynongController.class);
	...
	public ModelAndView createmynong(ModelAndView mv, HttpSession session, HttpServletResponse response)
		...
		if (id == null) {
			...
			int pan = mynongservice.checkmyfarm(id); // 농장 관리자일시 1, 농장 구성원이면 2
	...
	public ModelAndView createmynongrocess(Member member, ModelAndView mv, HttpServletRequest request) {
		logger.info("입력한 내농장 이름 :" + member.getName());
		logger.info("넘어온 ID :" + member.getId());

		int result = mynongservice.insertName(member.getMynong_name());
		mynongservice.insertNongName(member);
		if (result == 0) {
	...
	public int namecheck(String mynong_name) {
		int result = mynongservice.isName(mynong_name);
		return result;
	}
