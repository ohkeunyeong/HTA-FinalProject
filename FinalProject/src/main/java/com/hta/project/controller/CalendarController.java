...
public class CalendarController {
	...
	// 스케줄
	@GetMapping("/calendar")
	public ModelAndView calendar(String name, ModelAndView mv, HttpServletRequest request, HttpSession session,
	...
	public ModelAndView insertCalBoard(MyCalendar calendar, HttpSession session, ModelAndView mv, String name,
		...
		try { // 유효성 검사를 위한 초기 세팅
			...
			if (level == 1 && ((!(getmynong.equals(name))) || id == null) || list.getMy_farm().equals("3")) { // 다른 아이디
				...
				String mdate = calendar.getYear() + isTwo(calendar.getMonth()) + isTwo(calendar.getDate())
						+ isTwo(calendar.getHour()) + isTwo(calendar.getMin());
				calendar.setMdate(mdate);
				boolean isS = calservice.insertCal(calendar);

				if (isS) {
					mv.setViewName("redirect:calendar?name=" + name + "&year=" + calendar.getYear() + "&month="
							+ calendar.getMonth());
				} else {
					mv.setViewName("oky/error/error");
					mv.addObject("url", request.getRequestURL());
	...
	public ModelAndView calDetail(int seq, MyCalendar calendar, HttpSession session, ModelAndView mv, String name,
			HttpServletRequest request) {
	...
	public ModelAndView calDetailmain(int seq, MyCalendar calendar, HttpSession session, ModelAndView mv, String name,
			HttpServletRequest request) {
	...
	public ModelAndView updateForm(int seq, ModelAndView mv, String name, HttpSession session,
	...
	public ModelAndView calUpdate(int seq, ModelAndView mv, String name, HttpSession session,
			HttpServletRequest request, MyCalendar calendar) {
