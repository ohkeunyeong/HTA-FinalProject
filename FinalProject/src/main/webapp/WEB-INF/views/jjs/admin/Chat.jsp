	...
	<head>
		...
		<script>
			$(function(){
				...
				$.ajax({
					type : "post",
					url : "/project/member/memberChatList",
					dataType : 'json',
					success : function(data){
						console.log(data);
						if(data.memberCount > 0){
							$(".contacts").remove();
							var output = "<ul class='contacts'>";
							$(data.memberChatList).each(function(index, item){
								console.log(item.nick)
								output += "<li><div class='d-flex bd-highlight'>";
								output += "<div class='img_cont'>";
								output += "<img src='https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg' class='rounded-circle user_img'>";
								output += "<span class='online_icon'></span></div>";
								output += "<div class='user_info'>";
								output += "<span>" + item.nick + "</span>"
								output += "<p>"+ item.nick +" is online</p></div></div></li>";
							});
							output += "</ul>";
							$('.contacts_body').append(output);
						}
					}
				});
			});
